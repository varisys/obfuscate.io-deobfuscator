...
export default {
  ...
  async run(ast, state, sandbox) {
    ...
    // Control-flow flattening, dead-code injection, and object wrappers can be
    // enabled independently from string-array encoding in obfuscator.io. Keep
    // running the structural passes even when there is no string array.
    if (stringArray) {
